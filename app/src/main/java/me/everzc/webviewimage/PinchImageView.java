...
public class PinchImageView extends ImageView {
    ...
    private GestureDetector mGestureDetector = new GestureDetector(PinchImageView.this.getContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if (mPinchMode == PINCH_MODE_FREE && !(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                ...
                LogUtils.e("mGestureDetector  onFling 判断中间");
            }
            LogUtils.e("mGestureDetector  onFling");
            return true;
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            ...
            if (mPinchMode == PINCH_MODE_SCALE) {
                LogUtils.e(" //如果之前是缩放模式,还需要触发一下缩放结束动画");
                scaleEnd();
            }
            mPinchMode = PINCH_MODE_FREE;
            // LogUtils.e("ACTION_UP  return true;");
            ...
            if (!(mScaleAnimator != null && mScaleAnimator.isRunning())) {
               ...
               // LogUtils.e("ACTION_DOWN   mScaleCenter " + mScaleCenter.toString());
               // doubleTap(event.getX(),event.getY());
                //return mG.onTouchEvent(event);
                //mG.onTouchEvent(event);
            } else {
                LogUtils.e("mScaleAnimator ==== null  isRunning() ");
               // return false;
            }
            ...
            LogUtils.e("非第一个点按下，关闭滚动模式，开启缩放模式，记录缩放模式的一些初始数据");
           //0.5 return true;
         ...
         //   LogUtils.e(" //ACTION_MOVE   return true");
           //0.5 return true;
        }
        mG.onTouchEvent(event);
        return true;
    ...
    private void doubleTap(float x, float y) {
        ...
        LogUtils.e("doubleTap");
        LogUtils.e("mScaleCenter.toString() : "+mScaleCenter.toString());
        ...
        if (testBound.right - testBound.left < displayWidth) {
            ...
            LogUtils.e(" testBound.right - testBound.left < displayWidth");
        } else if (testBound.left > 0) {
            postX = -testBound.left;
            LogUtils.e("  testBound.left > 0");
        } else if (testBound.right < displayWidth) {
            postX = displayWidth - testBound.right;
            LogUtils.e(" testBound.right < displayWidth");
        }
        if (testBound.bottom - testBound.top < displayHeight) {
            LogUtils.e(" testBound.bottom - testBound.top < displayHeight");
            postY = displayHeight / 2f - (testBound.bottom + testBound.top) / 2f;
        } else if (testBound.top > 0) {
            postY = -testBound.top;
            LogUtils.e("testBound.top > 0");
        } else if (testBound.bottom < displayHeight) {
            LogUtils.e("testBound.bottom < displayHeight");
    ...
    private void scaleEnd() {
        ...
        if (currentScale > maxScale) {
            ...
            LogUtils.e("//如果整体缩放比例大于最大比例，进行缩放修正");
        }
